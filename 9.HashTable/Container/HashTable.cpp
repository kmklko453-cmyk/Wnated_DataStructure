...
// 원래 자료형 안에 선언되어 있는 함수 멤버 함수로 구현 할 필요가 없음 교육 목적의 구현
// 따라서 다른 클래스에서 접근을 막기위한 static 
...
bool HashTable::Add(const std::string& key, const std::string& value)
{
	...
	int bucketindex =  GenerateHash(key) % buketCount;
...
{
	// 해시 값을 생성한 후에 이를 버킷 인덱스로 변환
	int buketindex = GenerateHash(key) % buketCount;
